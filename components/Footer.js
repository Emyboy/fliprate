import React from "react";

export default function Footer() {
    return (
        <footer id='pq-footer' className='style-1'>
            <div className='pq-footer-style-1'>
                <div className='pq-footer-top'>
                    <div className='pq-footer-bottom-list'>
                        <div className='container'>
                            <div className='row g-0'>
                                <div className='col-lg-4 p-0'>
                                    <div className='pq-footer-items'>
                                        <i className='ti-location-pin'></i>
                                        <div className='pq-footer-items-info'>
                                            <h4>Address</h4>
                                            <span>
                                                Kiambere road building, Nachu
                                                Plaza, Nairobi Kenya.{" "}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                <div className='col-lg-4 p-0'>
                                    <div className='pq-footer-items'>
                                        <i className='ti-mobile'></i>
                                        <div className='pq-footer-items-info'>
                                            <h4>call Us On</h4>
                                            <a href='tel:+254 [0] 114569838'>
                                                <span>+254 [0] 114569838</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                                <div className='col-lg-4 p-0'>
                                    <div className='pq-footer-items border-0'>
                                        <i className='ti-email'></i>
                                        <div className='pq-footer-items-info'>
                                            <h4>Email Us</h4>
                                            <a href='mailto:info@flip.co.ke'>
                                                <span>info@flip.co.ke</span>
                                            </a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    {/* <div className="container">
                    <div className="row">
                        <div className="col-xl-3 col-md-6">
                            <div className="widget">
                                <img src="images/logos/footer-logo.png" className="pq-footer-logo" alt="gym-footer-logo"/>
                                <p>There are many variations of passages of Lorem Ipsum available, but the majority have
                                    suffered.</p>
                                <div className="pq-footer-social">
                                    <ul>
                                        <li><a href="#"><i className="fab fa-facebook-f"></i></a></li>
                                        <li><a href="#"><i className="fab fa-github"></i></a></li>
                                        <li><a href="#"><i className="fab fa-instagram"></i></a></li>
                                        <li><a href="#"><i className="fab fa-skype"></i></a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div className="col-xl-3  col-md-6">
                            <div className="widget">
                                <h4 className="footer-title">Our Services</h4>
                                <div className="menu-useful-service-links-container">
                                    <ul id="menu-useful-service-links" className="menu">
                                        <li>
                                            <a href="software-development.html">Software Development</a>
                                        </li>
                                        <li>
                                            <a href="web-development.html">Web Development</a>
                                        </li>
                                        <li>
                                            <a href="mobile-development.html">Mobile Development</a>
                                        </li>
                                        <li>
                                            <a href="app-development.html">App Development</a>
                                        </li>
                                        <li>
                                            <a href="qa-testing.html">QA Testing</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div className="col-xl-3  col-md-6">
                            <div className="widget">
                                <h4 className="footer-title">Useful link</h4>
                                <div className="menu-useful-link-container">
                                    <ul id="menu-usefull-link" className="menu">
                                        <li>
                                            <a href="#">Fact Search Engine</a>
                                        </li>
                                        <li>
                                            <a href="#">Learn and Code</a>
                                        </li>
                                        <li>
                                            <a href="#">Video Sharing Website</a>
                                        </li>
                                        <li>
                                            <a href="#">File &amp; URL Scanner</a>
                                        </li>
                                        <li>
                                            <a href="#">Online Software Package</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div className="col-xl-3  col-md-6">
                            <div className="pq-widget-menu widget">
                                <h4 className="footer-title">Recent post</h4>
                                <div className="pq-footer-recent-post">
                                    <div className="pq-footer-recent-post-media">
                                        <a href="app-promotes-sales-profits-in-it-solutions.html">
                                            <img src="images/footer-blog/1.jpg" alt=""/>
                                        </a>
                                    </div>
                                    <div className="pq-footer-recent-post-info">
                                        <h6><a href="app-promotes-sales-profits-in-it-solutions.html">App promotes sales
                                                &amp; profits In It Solutions</a></h6>
                                        <a href="app-promotes-sales-profits-in-it-solutions.html" className="pq-post-date">January <span>26</span>, 2021 </a>
                                    </div>
                                </div>
                                <div className="pq-footer-recent-post">
                                    <div className="pq-footer-recent-post-media">
                                        <a href="app-promotes-sales-profits-in-it-solutions.html">
                                            <img src="images/footer-blog/2.jpg" alt=""/>
                                        </a>
                                    </div>
                                    <div className="pq-footer-recent-post-info">
                                        <h6><a href="app-promotes-sales-profits-in-it-solutions.html">In Depth Industry
                                                &amp; Ecosystem Analysis</a></h6>
                                        <a href="app-promotes-sales-profits-in-it-solutions.html" className="pq-post-date">January <span>26</span>, 2021 </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div> */}
                </div>
                <div className='pq-copyright-footer'>
                    <div className='container'>
                        <div className='row'>
                            <div className='col-lg-6 col-md-6'>
                                <span className='pq-copyright'>
                                    <a target='_blank' href='#'>
                                        {" "}
                                        Copyright 2022 Fliprate All Rights
                                        Reserved.
                                    </a>
                                </span>
                            </div>
                            <div className='col-lg-6 col-md-6'>
                                <div className='pq-footer-link'>
                                    {/* <ul>
                    <li>
                      <a href="#">Career</a>
                    </li>
                    <li>
                      <a href="#">Term Of Use</a>
                    </li>
                    <li>
                      <a href="#">Privacy Policy</a>
                    </li>
                  </ul> */}
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </footer>
    );
}
